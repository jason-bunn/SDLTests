...
{
	// if joystick subsystem uninitialized, do so now
	if (SDL_WasInit(SDL_INIT_JOYSTICK) == 0)
	{
		SDL_InitSubSystem(SDL_INIT_JOYSTICK);
	}

	//get the number of joysticks, skip if none
	if (SDL_NumJoysticks() > 0)
	{
		for (int i = 0; i < SDL_NumJoysticks(); i++)
		{
			...
			//if opened correctly, populate vector
			if (joy != 0)
			{
				...
				//create a vector to hold button values
				std::vector<bool> tempButtons;

				//fill vector with false values for each button
				for (int j = 0; j < SDL_JoystickNumButtons(joy); j++)
				{
					tempButtons.push_back(false);
				}

				//push button vector into button state vector
				m_buttonStates.push_back(tempButtons);
			}
			else
			{
				//if there was an error initializing a joystick we want to know
...
//direct events to appropriate handler
